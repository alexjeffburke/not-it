...
module.exports = function () {
    ...
    if (numberOfArguments === 1) {
        if (typeof globalIt !== 'function') {
            throw new Error('Must be invoked on a function block.');
        }

        if (typeof globalIt.skip === 'function') {
            // wrapping invocation
            return executeIt.bind(null, globalIt);
        } else {
            // nested invocation
            return executeWithinIt.bind(null, globalIt);
        }
